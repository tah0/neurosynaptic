#!/usr/bin/env python3
"""Transform Current_Source.kicad_sch: replace ALD1105PBL with BJT equivalents."""

import uuid, re, sys

SCH   = "/home/todd/Documents/Thesis/neuromorphic circuit/neurosynaptic/PCBdesign/kicad/CurrentSource/Current_Source.kicad_sch"
DELTA = "/home/todd/Documents/Thesis/neuromorphic circuit/neurosynaptic/PCBdesign/kicad/Synapse/Delta.kicad_sch"
...
with open(DELTA) as f: delta = f.read()
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 1: lib_symbols – remove ALD1105PBL, add Q_NPN and Q_PNP_CurrentMirror
# ══════════════════════════════════════════════════════════════════════════════
...
end_marker = '\t\t\t(embedded_fonts no)\n\t\t)'

def extract_from_delta(sym_name):
    start_str = f'\t\t(symbol "{sym_name}"\n'
    idx_start = delta.find(start_str)
    if idx_start < 0:
        errors.append(f"Delta lib sym not found: {sym_name}"); sys.exit(1)
    idx_end = delta.find(end_marker, idx_start)
    return delta[idx_start : idx_end + len(end_marker)]

q_npn_lib    = extract_from_delta('Device:Q_NPN')
q_pnp_cm_lib = extract_from_delta('Device:Q_PNP_CurrentMirror')

new_lib = q_npn_lib + '\n' + q_pnp_cm_lib
c = c[:ald_match.start()] + new_lib + c[ald_match.end():]
print("✓ lib_symbols: replaced ALD1105PBL with Device:Q_NPN + Device:Q_PNP_CurrentMirror")
...
q1 = bjt("Device:Q_PNP_CurrentMirror", 165.1,  108.4072, 0, "Q1", "BC557", ["C1","C2","E1","E2"])
q2 = bjt("Device:Q_NPN",               149.86, 121.1072, 0, "Q2", "BC547", ["B","C","E"])

vcc_e1 = vcc_sym(170.18, 103.3272)   # E1 of Q1
vcc_e2 = vcc_sym(160.02, 103.3272)   # E2 of Q1

new_elements = '\n'.join([
    wire(170.18, 113.4872, 190.5,  113.4872),   # Q1 C1 → J2 input
    wire(160.02, 113.4872, 142.24, 113.4872),   # Q1 C2 → output column
    wire(152.4,  116.0272, 152.4,  118.5672),   # Q2 C down
    wire(152.4,  118.5672, 142.24, 118.5672),   # Q2 C → SN2 junction
    wire(152.4,  126.1872, 152.4,  128.7272),   # Q2 E → GND junction
])

new_syms = '\n'.join([q1, q2, vcc_e1, vcc_e2])
...
if c.rstrip().endswith(')'):
    ...
    if c[-1] != '\n':
        c += '\n'
    print("✓ added Q1/Q2 BJTs, 5 wires, 2 +5V symbols")
else:
...
closes = c.count(')')
if opens != closes:
    print(f"\n⚠  Paren mismatch: {opens} open, {closes} close")
    sys.exit(1)

with open(SCH, 'w') as f:
    f.write(c)
print(f"\n✓ Current_Source.kicad_sch written ({len(c)} chars)")
print(f"  Paren balance: {opens} opens = {closes} closes ✓")
