#!/usr/bin/env python3
"""Transform Delta.kicad_sch: replace ALD1105PBL with BJT equivalents."""

import uuid, re, sys

SCH   = "/home/todd/Documents/Thesis/neuromorphic circuit/neurosynaptic/PCBdesign/kicad/Synapse/Delta.kicad_sch"
CM1   = "/home/todd/Documents/Thesis/neuromorphic circuit/neurosynaptic/PCBdesign/kicad/Synapse/Current Mirror_1.kicad_sch"
...
with open(CM1) as f:  cm1 = f.read()
with open(DEV) as f:  dev = f.read()

errors = []

# ─── Generic block removal by UUID ────────────────────────────────────────────
...
# ─── BJT instance ─────────────────────────────────────────────────────────────
def bjt(lib_id, cx, cy, rot, ref, value, pin_names):
    if rot == 0:
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 1: lib_symbols – remove ALD1105PBL, add Q_NPN/Q_PNP/Q_NPN_CM/Q_PNP_CM
# ══════════════════════════════════════════════════════════════════════════════
...
q_npn_lib = extract_dev_sym('Q_NPN')
q_pnp_lib = extract_dev_sym('Q_PNP')

idx_cm_npn  = cm1.find('\t\t(symbol "Device:Q_NPN_CurrentMirror"\n')
idx_cm_pnp  = cm1.find('\t\t(symbol "Device:Q_PNP_CurrentMirror"\n')
idx_cm_pend = cm1.rfind('\t\t\t(embedded_fonts no)\n\t\t)')

q_npn_cm_lib = cm1[idx_cm_npn : cm1.find('\t\t\t(embedded_fonts no)\n\t\t)', idx_cm_npn) + len('\t\t\t(embedded_fonts no)\n\t\t)')]
q_pnp_cm_lib = cm1[idx_cm_pnp : idx_cm_pend + len('\t\t\t(embedded_fonts no)\n\t\t)')]

new_lib = q_npn_lib + '\n' + q_pnp_lib + '\n' + q_npn_cm_lib + '\n' + q_pnp_cm_lib
c = c[:ald_match.start()] + new_lib + c[ald_match.end():]
print("✓ lib_symbols: replaced ALD1105PBL with Q_NPN/Q_PNP/Q_NPN_CM/Q_PNP_CM")
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 4: Modify wires (change endpoint coordinates)
# ══════════════════════════════════════════════════════════════════════════════

# Shorten GP1 loop vertical: 60.1472 → 57.6072
...
# Extend output path upward: 90.6272 → 80.4672
...
new_elements = '\n'.join([
    # IC1 area
    wire(76.2, 55.0672, 76.2, 47.4472),      # Q3 C → node_A
    wire(76.2, 47.4472, 78.74, 47.4472),      # node_A horizontal
    wire(86.36, 62.6872, 68.58, 62.6872),     # Q5 C → SP1 path
    # IC? area
    wire(238.76, 85.5472, 238.76, 88.0872),   # Q7 C1 down
    wire(238.76, 88.0872, 228.6, 88.0872),    # Q7 C1 → GND bus
    wire(238.76, 75.3872, 251.46, 75.3872),   # cascade horizontal
    wire(251.46, 75.3872, 251.46, 80.4672),   # cascade → Q8 E2
    wire(241.3, 80.4672, 226.06, 80.4672),    # Q8 E1 → output left
    wire(241.3, 90.6272, 259.08, 90.6272),    # Q8 C1/C2 → +5V bus
    wire(259.08, 90.6272, 259.08, 105.8672),  # +5V bus → +5V_BAR
    # Junctions
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 8: Add new BJT instances and power symbols
# ══════════════════════════════════════════════════════════════════════════════

q3   = bjt("Device:Q_NPN",              73.66, 49.9872, 0,   "Q3", "BC547", ["B","C","E"])
q5   = bjt("Device:Q_PNP",              83.82, 57.6072, 0,   "Q5", "BC557", ["B","C","E"])
q7   = bjt("Device:Q_NPN_CurrentMirror",233.68, 80.4672, 0,  "Q7", "BC547", ["C1","C2","E1","E2"])
q8   = bjt("Device:Q_PNP_CurrentMirror",246.38, 85.5472, 180,"Q8", "BC557", ["C1","C2","E1","E2"])
gnd3 = gnd_sym(76.2, 44.9072)           # Q3 E → GND
vcc5 = vcc_sym(86.36, 52.5272, rot=180) # Q5 E → +5V

new_syms = '\n'.join([q3, q5, q7, q8, gnd3, vcc5])

# Append before the final closing paren
if c.rstrip().endswith(')'):
    last_nl = c.rfind('\n)')
    c = c[:last_nl] + '\n' + new_syms + '\n)'
    if c[-1] != '\n':
        c += '\n'
    print("✓ added Q3/Q5/Q7/Q8 + GND/+5V symbol instances")
else:
...
with open(SCH, 'w') as f:
    f.write(c)
print(f"\n✓ Delta.kicad_sch written ({len(c)} chars)")
